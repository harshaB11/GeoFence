...
class ListOfDeviceActivity : AppCompatActivity() , ActivityCompat.OnRequestPermissionsResultCallback,
    OnMapReadyCallback,
    MapboxMap.OnMapClickListener{
    companion object{
        private const val MARKER_TRACKER = "MARKER_TRACKER12"
        private const val MARKER_SELECTED_TRACKER = "MARKER_SELECTED_TRACKER12"
    }

    lateinit var binding: ActivityListOfDeviceBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding=
            DataBindingUtil.setContentView(this, R.layout.activity_list_of_device)
        binding.mapView.onCreate(savedInstanceState)
        binding.mapView.getMapAsync(this)
        binding.deleteTracker.setOnClickListener {
                            ...
                            ThreadUtils.runOnUiThread {
                                ...
                                for (list in response.entries) {
                                    insertTrackerIconOnMap(
                                        ...
                                        "Time : " + list.sampleTime
                                    )
                                    mapboxMap?.cameraPosition = CameraPosition.Builder()
                                        .target(LatLng(response.entries.get(0).position.get(1),
                                            response.entries.get(0).position.get(0)))
                                        .zoom(18.0)
                                        .build()
    ...
    private fun insertTrackerIconOnMap(point: LatLng, title: String, imageId: Int, description: String) {
